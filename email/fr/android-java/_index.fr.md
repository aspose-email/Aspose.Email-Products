---
title: API de messagerie Android | Traitement du calendrier et analyse MIME 
weight: 2450
url: /fr/android-java/ 
description: La bibliothèque de messagerie permet d'accéder, de lire et de manipuler les fichiers Outlook MSG PST OST EML MHT dans les applications mobiles.
---

{{< blocks/products/pf/main-wrap-class >}}
{{< blocks/products/pf/i18n/upper-banner h1="API de messagerie Android" h2="Créez, manipulez et convertissez des e-mails et des fichiers de stockage, y compris MSG, EML, MHT, PST et OST, à partir d'applications mobiles." logoImageSrc="https://www.aspose.cloud/templates/aspose/img/products/email/headers/aspose_email-for-android-java.svg" pfName="Aspose.Email" subTitlepfName="for Android via Java" downloadUrl="https://downloads.aspose.com/email/androidjava" >}}

{{< blocks/products/pf/main-container pfName="Aspose.Email" subTitlepfName="for Android via Java" >}}

{{< blocks/products/pf/i18n/sub-menu logoImageSrc="https://www.aspose.cloud/templates/aspose/img/products/email/aspose_email-for-android-java.svg" liveDemosLink="" PricingLink="https://purchase.aspose.com/pricing/email/android-java" buyLink="https://purchase.aspose.com/buy" docsLink="https://docs.aspose.com/email/java/android-via-java-release-notes/" installationsDocsLink="https://docs.aspose.com/email/java/aspose-email-for-android-via-java-installation/" nugetLink="" nugetPackageName="Microsoft Outlook:" mavenRepoLink="https://repository.aspose.com/webapp/#/artifacts/browse/tree/General/repo/com/aspose/aspose-email" directDownloadLink="https://downloads.aspose.com/email/androidjava" >}}

{{< blocks/products/pf/i18n/tab-content >}}
<p>
 Aspose.Email pour Android via Java permet aux développeurs de logiciels de concevoir des applications Android capables de gérer et de manipuler des messages électroniques sans se perdre dans les complexités de la mise en œuvre du format de fichier Microsoft Outlook. La bibliothèque de messagerie Android aide les développeurs à manipuler les formats de fichiers de messagerie à partir des applications Android. L'API n'a aucune dépendance autre qu'Android Runtime, elle n'a donc pas besoin d'autres API ou logiciels pour fonctionner.
</p>

<p>
 Aspose.Email pour Android via Java vous donne les outils dont vous avez besoin pour créer, lire et manipuler les fichiers Outlook MSG, PST, EML et MHT à partir d'une application Android. Il offre la possibilité de manipuler les en-têtes de message, le contenu, les pièces jointes et les propriétés Mapi conformément aux spécifications de l'application.
</p>

{{< /blocks/products/pf/i18n/tab-content >}}

<!--Diagrams Start-->
{{< blocks/products/pf/i18n/carousel >}}

{{< blocks/products/pf/carousel-item h3="En bref" description="Un aperçu de l'API." >}}
<div class="diagram1 d1-android">
 <div class="d1-row">
  <div class="d1-col d1-left">
   <header>
    <i class="fa fa-archive">
    </i>
    Stockage
   </header>
   <ul>
    <li>
     Create PST and OST Files
    </li>
    <li>
     Gérer les messages dans PST
    </li>
    <li>
     Gérer les contacts dans PST
    </li>
    <li>
     Gérer les éléments de calendrier dans PST
    </li>
    <li>
     Gérer les tâches, les notes, les journaux
    </li>
   </ul>
   <header>
    <i class="fa fa-calendar">
    </i>
    Événements
   </header>
   <ul>
    <li>
     Modèles de récurrence
    </li>
    <li>
     Créer un rendez-vous
    </li>
    <li>
     Exporter le rendez-vous vers le fichier ICS
    </li>
    <li>
     Formater le texte du rendez-vous
    </li>
    <li>
     Lire les informations sur le rendez-vous
    </li>
   </ul>
  </div>
  <!--/left-->
  <div class="d1-col d1-right">
   <header>
    <i class="fa fa-envelope">
    </i>
    E-mails
   </header>
   <ul>
    <li>
     Charger une variété de formats
    </li>
    <li>
     Interconvertir les messages
    </li>
    <li>
     Create Plain and HTML E-mails
    </li>
    <li>
     Gérer les objets intégrés
    </li>
    <li>
     Gérer les pièces jointes
    </li>
    <li>
     Enregistrer le message comme brouillon
    </li>
    <li>
     Gérer les en-têtes
    </li>
   </ul>
  </div>
  <!--/right-->
 </div>
 <!--/row-->
 <div class="d1-logo">
  <img width="70" height="75" alt="API de messagerie Android" src="https://www.aspose.cloud/templates/aspose/img/products/email/aspose_email-for-android-java.svg"/>
  <header>
   Aspose.Email
  </header>
  <footer>
   <small>
    <em>
     for
    </em>
    Android via Java
   </small>
  </footer>
 </div>
 <!--/logo-->
</div>

{{< /blocks/products/pf/carousel-item >}}

{{< blocks/products/pf/carousel-item h3="Indépendance de la plate-forme" description="Aspose.Email s'exécute sur n'importe quelle plate-forme sur laquelle l'environnement d'exécution Android est installé." >}}
<div class="diagram1 d1-android">
 <div class="d1-row">
  <div class="d1-col d1-left">
  </div>
  <!--/left-->
  <div class="d1-col d1-right">
   <header style="padding-left: 0px;">
    <i class="fa fa-cogs">
    </i>
    Environnement d'exécution Android
   </header>
   <ul>
    <li>
     Application mobile
    </li>
   </ul>
  </div>
  <!--/right-->
 </div>
 <!--/row-->
 <div class="d1-logo">
  <img width="70" height="75" alt="API de messagerie Outlook" src="https://www.aspose.cloud/templates/aspose/img/products/email/aspose_email-for-android-java.svg"/>
  <header>
   Aspose.Email
  </header>
  <footer>
   <small>
    <em>
     for
    </em>
    Android via Java
   </small>
  </footer>
 </div>
 <!--/logo-->
</div>

{{< /blocks/products/pf/carousel-item >}}

{{< blocks/products/pf/carousel-item h3="Formats de fichiers pris en charge" description="L'API prend en charge les formats de fichiers de messages et de stockage les plus populaires." >}}
<div class="diagram1 d2 d1-android">
 <div class="d1-row">
  <div class="d1-col d1-left">
   <header>
    <i class="fa fa-arrows-v">
    </i>
    Entrée sortie
   </header>
   <ul>
    <li>
     <b>
      Microsoft Outlook:
     </b>
     MSG, PST, OST
    </li>
    <li>
     EML
    </li>
   </ul>
  </div>
  <!--/left-->
  <div class="d1-col d1-right">
  </div>
  <!--/row-->
  <div class="d1-logo">
   <img width="70" height="75" alt="API de messagerie pour Android" src="https://www.aspose.cloud/templates/aspose/img/products/email/aspose_email-for-android-java.svg"/>
   <header>
    Aspose.Email
   </header>
   <footer>
    <small>
     <em>
      for
     </em>
     Android via Java
    </small>
   </footer>
  </div>
  <!--/logo-->
 </div>
 <!--/diagram3-->
</div>

{{< /blocks/products/pf/carousel-item >}}

{{< /blocks/products/pf/i18n/carousel >}}
<!--Diagrams End-->

<!--Feature-section Start-->
<div class="container-fluid features-section bg-gray singleproduct">
 <a class="anchor" id="features" name="features">
 </a>
 <div class="row">
  <div class="container">
   <h2 class="pr-ft">
    Advanced API de messagerie Android Features
   </h2>
   <p>
   </p>
   <div class="col-lg-4">
    <em class="fa fa-file-text-o ico-blue fa-2x col-lg-2">
    </em>
    <p class="col-lg-10">
     Importer et exporter un format de message conforme à RFC822
    </p>
   </div>
   <div class="col-lg-4">
    <em class="fa fa-envelope-o ico-blue fa-2x col-lg-2">
    </em>
    <p class="col-lg-10">
     Lire et manipuler différents types de messages via l'API du produit
    </p>
   </div>
   <div class="col-lg-4">
    <em class="fa fa-paperclip ico-blue fa-2x col-lg-2">
    </em>
    <p class="col-lg-10">
     Ajouter, supprimer ou extraire des pièces jointes aux messages
    </p>
   </div>
   <div class="col-lg-4">
    <em class="fa fa-edit ico-blue fa-2x col-lg-2">
    </em>
    <p class="col-lg-10">
     Manipuler des objets intégrés
    </p>
   </div>
   <div class="col-lg-4">
    <em class="fa fa-code ico-blue fa-2x col-lg-2">
    </em>
    <p class="col-lg-10">
     Calculer les occurrences de date et d'heure pour les modèles de récurrence complexes
    </p>
   </div>
   <div class="col-lg-4">
    <em class="fa fa-calendar ico-blue fa-2x col-lg-2">
    </em>
    <p class="col-lg-10">
     Produire et consommer des modèles de récurrence dans iCalendar (RFC 2445)
    </p>
   </div>
   <div class="col-lg-4">
    <em class="fa fa-save ico-blue fa-2x col-lg-2">
    </em>
    <p class="col-lg-10">
     Personnaliser les en-têtes
    </p>
   </div>
   <div class="col-lg-4">
    <em class="fa fa-cogs ico-blue fa-2x col-lg-2">
    </em>
    <p class="col-lg-10">
     Charger et analyser les e-mails aux formats MSG, MHT et EML
    </p>
   </div>
   <div class="col-lg-4">
    <em class="fa fa-database ico-blue fa-2x col-lg-2">
    </em>
    <p class="col-lg-10">
     Extraire les messages des fichiers de stockage
    </p>
   </div>
   <!--

<div class="col-lg-4"><em class="fa fa-group ico-blue fa-2x col-lg-2"> </em>

<p class="col-lg-10">Add or extract messages & contacts from PST and OST files</p>

</div>



<div class="col-lg-4"><em class="fa fa-calendar-plus-o  ico-blue fa-2x col-lg-2"> </em>

<p class="col-lg-10">Adding and saving calendar items from PST files</p>

</div>

<div class="col-lg-4"><em class="fa fa-sticky-note ico-blue fa-2x col-lg-2"> </em>

<p class="col-lg-10">Adding tasks, notes and journals to PST files</p>

</div>

-->
   <div class="col-lg-12">
    <h2 class="h2title">
     Conversion de MSG en différents formats
    </h2>
    <p>
     La bibliothèque facilite la conversion de différents formats de fichiers de messages. Le processus de conversion consiste à charger le fichier source dans le modèle d'objet de l'API et à appeler la fonction de sauvegarde avec les paramètres pertinents. C'est vraiment aussi simple!
    </p>
    <div class="codeblock" id="code">
     <h3>
      Enregistrer le message dans différents formats - Java
     </h3>
     <pre><code class="java">// charge le document pour la conversion

MailMessage msge = MailMessage.load(SD_PATH + "template.msg");

// Convertir MSG en EML, HTML et MHTML

msge.save(SD_PATH + "output.eml", SaveOptions.getDefaultEml());

msge.save(SD_PATH + "output.html", SaveOptions.getDefaultHtml());

msge.save(SD_PATH + "output.mhtml", SaveOptions.getDefaultMhtml());</code></pre>
    </div>
   </div>
   <div class="col-lg-12">
    <h2 class="h2title">
     Lire les propriétés Outlook MAPI
    </h2>
    <p>
     La bibliothèque Android Outlook vous permet de lire les propriétés MAPI du fichier MSG. Il prend en charge la lecture de toutes les propriétés MAPI ou la lecture d'une propriété spécifique à l'aide de la balise de propriété MAPI.
    </p>
   </div>
   <div class="col-lg-12">
    <h2 class="h2title">
     Travailler avec l'API Android iCalendar
    </h2>
    <p>
     L'API iCalendar d'Aspose.Email permet de créer et d'enregistrer des objets de calendrier Outlook dans plusieurs formats tels que MSG et ICS. Vous pouvez également lire des objets de calendrier à l'aide de cette API.
    </p>
   </div>
   <div class="col-lg-12">
    <h2 class="h2title">
     Read Outlook PST File
    </h2>
    <p>
     La bibliothèque vous permet de lire et d'écrire un fichier Microsoft Outlook PST. Il donne accès à tous les dossiers et sous-dossiers d'un fichier PST afin que vous puissiez ajouter, lire et extraire des messages. Les messages, contacts et éléments de calendrier peuvent être exportés et enregistrés sur disque dans différents formats à partir d'un fichier PST.
    </p>
   </div>
   <div class="col-lg-12">
    <h2 class="h2title">
     Gérer les pièces jointes
    </h2>
    <p>
     L'API Mail offre la possibilité de lire et d'extraire les pièces jointes des fichiers de messages d'une manière très simple. Non seulement vous pouvez extraire des pièces jointes, mais également créer de nouvelles pièces jointes et les ajouter aux messages.
    </p>
   </div>
   <!--

<div class="col-lg-12">

<h2 class="h2title">Microsoft Office Automation – Not Needed</h2>

<p>Aspose.Email for Android via Java API is built using managed code that do never need Microsoft Office or Microsoft Outlook to be installed on the machine to work with emails processing. It is a perfect Microsoft Outlook automation alternative in terms of supported features, security, stability, scalability, speed and price.</p>

</div>

-->
  </div>
 </div>
</div>
<!--Feature-section End-->

{{< /blocks/products/pf/main-container >}}


{{< blocks/products/pf/i18n/support-learning-resources >}}
{{< blocks/products/pf/slr-tab tabTitle="Ressources d'apprentissage" tabId="resources" >}}
{{< blocks/products/pf/slr-element name="Documentation" href="https://docs.aspose.com/email/androidjava/" >}}
{{< blocks/products/pf/slr-element name="Code source" href="https://github.com/aspose-email/Aspose.Email-for-Java" >}}
{{< blocks/products/pf/slr-element name="Références API" href="https://apireference.aspose.com/email" >}}
{{< blocks/products/pf/slr-element name="Vidéos didactiques" href="https://www.youtube.com/user/asposevideo" >}}
{{< /blocks/products/pf/slr-tab >}}

{{< blocks/products/pf/slr-tab tabTitle="Support produit" tabId="support" >}}
{{< blocks/products/pf/slr-element name="Assistance gratuite" href="https://forum.aspose.com/c/email" >}}
{{< blocks/products/pf/slr-element name="Assistance payante" href="https://helpdesk.aspose.com/" >}}
{{< blocks/products/pf/slr-element name="Blog" href="https://blog.aspose.com/category/email/" >}}
{{< /blocks/products/pf/slr-tab >}}

{{< blocks/products/pf/slr-tab tabTitle="Pourquoi Aspose.Email pour Android via Java ?" tabId="success-stories" >}}
{{< blocks/products/pf/slr-element name="Liste des clients" href="https://company.aspose.com/customers" >}}
{{< blocks/products/pf/slr-element name="Réussites" href="https://company.aspose.com/customers/success-stories/aspose-email" >}}
{{< /blocks/products/pf/slr-tab >}}

{{< /blocks/products/pf/i18n/support-learning-resources >}}

{{< blocks/products/pf/i18n/download-section downloadFreeTrialLink="https://downloads.aspose.com/email/androidjava" pricingInformationLink="https://purchase.aspose.com/pricing/email/android-java" >}}

{{< blocks/products/pf/offers-section pfName="Aspose.Email " description="Aspose.Email propose des API de messagerie individuelles pour d'autres environnements de développement populaires, comme indiqué ci-dessous :" >}}

    {{< blocks/products/pf/offers-section-item link="/email/fr/net/" imgSrc="https://www.aspose.cloud/templates/aspose/img/products/email/aspose_email-for-net.svg" sdkName=".NET" >}}
    {{< blocks/products/pf/offers-section-item link="/email/fr/java/" imgSrc="https://www.aspose.cloud/templates/aspose/img/products/email/aspose_email-for-java.svg" sdkName="Java" >}}
    {{< blocks/products/pf/offers-section-item link="/email/fr/cpp/" imgSrc="https://www.aspose.cloud/templates/aspose/img/products/email/aspose_email-for-cpp.svg" sdkName="C++" >}}
    {{< blocks/products/pf/offers-section-item link="/email/fr/sharepoint/" imgSrc="https://www.aspose.cloud/templates/aspose/img/products/email/aspose_email-for-sharepoint.svg" sdkName="SharePoint" >}}
    {{< blocks/products/pf/offers-section-item link="/email/fr/python-net/" imgSrc="https://www.aspose.cloud/templates/aspose/img/products/email/aspose_email-for-python-net.svg" sdkName="Python via .NET" >}}

{{< /blocks/products/pf/offers-section >}}

{{< /blocks/products/pf/main-wrap-class >}}